...
import com.punam.employee.model.OrgnizationDto;
...
//				.bodyToMono(DepartmentDto.class).block();
		...
		OrgnizationDto orgnizationDto = webClient.get()
				.uri("http://localhost:8082/api/orgnizations/" + employee.getOrgnizationCode()).retrieve()
				.bodyToMono(OrgnizationDto.class).block();
		

		APIResponse response = new APIResponse();
		response.setEmployee(employeeDto);
		response.setDepartment(departmentDto);
		response.setOrgnization(orgnizationDto);
